  ...
  initTrackClicks() {
    document.getElementsByTagName("body")[0].addEventListener(
      "click", e => {
        let eventTarget;
        if (e.composedPath) {
          eventTarget = e.composedPath().find(el => {
            if (el.classList) {
              return el.classList.contains("js-track-click");
            }
            return false;
          });
        } else {
          ...
          do {
            ...
            !currentElement.classList.contains("js-track-click")
          );
          if (
            currentElement.classList &&
            currentElement.classList.contains("js-track-click")
          )
            eventTarget = currentElement;
        }
        ...
        if (eventTarget) {
          ...
          const eventInfo = {
            ...
            eventAction:target.getAttribute("data-action") || target.innerText.trim(),
            eventLabel:target.getAttribute("data-label") || window.location.pathname,
            eventCategory:target.getAttribute("data-eventCategory") || "button click"
          };
          this.addEvent(eventInfo);
        }
      },
